...
class Review extends Component {
  ...
  onFileUpload = (e) => {
    ...
    axios.post("http://127.0.0.1:5000/postcsv", formData).then((response) => {
      if (Math.round(response.data) === 2) {
        this.setState({ prediction: "Positive" });
      }
      if (Math.round(response.data) === 1) {
        this.setState({ prediction: "Neutral" });
      }
      if (Math.round(response.data) === 0) {
        this.setState({ prediction: "Negative" });
      }
    });
  };
